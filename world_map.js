(() => {
  ...
  const REGIONS = [
    // 中央 / 王都アウレリア
    { g: "central", name: "王都アウレリア中心部", manage: "王権/ギルド/教会/学院/商会", cond: "北門一時通行", needs: "商会紹介状 または 正式身分証", map: "plaza", spawn: { x: 0, z: 40 }, note: "4ブロック構想の対象(王都中心部のみ)。思想別地区ではなく段階拡張の制作単位", unlock: (c) => c.merchantLetter || c.done("gate") },
    { g: "central", name: "行政通り / 許可局窓口", manage: "王都行政/記録官/衛兵", cond: "北門一時通行", needs: "用途別許可(王城区・港湾・辺境砦など)", map: "administrativeStreet", spawn: { x: 0, z: 54 }, note: "Phase 2の軽量サブマップ。行政機能の周囲に住宅・小商店・市民生活も混在", unlock: (c) => c.merchantLetter || c.done("gate") },
